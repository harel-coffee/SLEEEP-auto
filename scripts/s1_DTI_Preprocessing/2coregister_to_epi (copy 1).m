% root of files to be registered
brains = '/data/projects/depredict/repositories/SLEEEP/freesurfer_working_directory/vols_BRAIN/';
% root of files with desired space
memprs = '/data/projects/depredict/repositories/SLEEEP/freesurfer_working_directory/vols_MEMPRAGE/';
% root of target directory
newbrs = '/data/projects/depredict/repositories/SLEEEP/freesurfer_working_directory/vols_BRAIN2/';

files = dir(fullfile(memprs, '*', '*.nii.gz'));
...
for ii=1:length(files)
   ...
   mempr=fullfile(memprs, sub, strcat(fname, '.nii.gz'));
   brain=fullfile(brains, sub, strcat(fname, '.nii.gz'));
   newbr=fullfile(newbrs, sub, strcat(fname, '.nii.gz'));

   if isfile(newbr)
       continue
   else
      coregister_to_epi(brain, mempr)
   end
end
